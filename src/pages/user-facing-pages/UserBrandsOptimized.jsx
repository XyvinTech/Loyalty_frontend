...
/**
 * Optimized UserBrands with Infinite Scroll
 * Reduces memory usage by loading brands in batches
 * Better for iOS webviews with limited memory
 */
const UserBrandsOptimized = () => {
  ...
  const {
    ...
    page,
    limit: 20, // Load 20 at a time instead of 100
  ...
  useEffect(() => {
    if (brands.length > 0) {
      ...
      setHasMore(brands.length === 20); // If we got less than 20, no more to load
    } else if (page === 1) {
      setAllBrands([]);
      setHasMore(false);
    }
  ...
  if (error && !isLoading && allBrands.length === 0) {
    return (
