...
const ArabicRedeemCard = ({ onClose, image }) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const msg = e?.data || "فشل في استبدال النقاط";
      setErrorMessage(msg);
      setShowPopup("error");
    } finally {
      setLoading(false);
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-8 flex flex-col items-center max-w-sm mx-4 text-center animate-fadeInUp relative border border-gray-100">
            ...
            {showPopup === "success" ? (
              <>
                ...
                <p className="font-medium text-[10px] text-gray-800 leading-7 alexandria-text text-right px-2">
                  مرحبا {customerName || "العميل"}،
                  <br />
                  تم استرداد قسيمتك بنجاح!
                  <br />
                  <span className="font-bold text-[#FF9900] bg-orange-50 px-2 py-1 rounded-md inline-block mt-2 mb-2">
                    رمز العملية: {transactionId || "---"}
                  </span>
                  <br />
                  للاستفادة من العرض أظهر هذه الرسالة لدى المتجر الشريك المعتمد
                  <br />
                  لمتابعة معاملاتك راجع قسم{" "}
